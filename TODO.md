...
1. [ ] **П1. Выделение оркестратора (рефакторинг без изменения поведения)**
...
2. [ ] **П2. Мини-слой за флагом (ошибки распознавания)**
   - `lib/mini.py` + флаг `mini.enabled=false` в конфиге.
   - rapidfuzz/диф против `commands.json.match`-фраз; кандидаты коррекции
     («сделай кромку пожалйста» → «сделай громче»).
   - Пока ВЫКЛЮЧЕН: при включённом флаге — только логирование кандидата,
     НЕ исполнение. TDD + 100%.
