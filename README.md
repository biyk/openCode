...
- Команды с ключевым словом (триггер) из `targets/<hostname>/commands.json`
...
```json
{
    ...
    "commands": {
        "volumeup": {
            "linux": "pactl set-sink-volume @DEFAULT_SINK@ +{{step}}%",
            "step": 6,
            "default": "pactl set-sink-volume @DEFAULT_SINK@ +{{step}}%"
        }
    },
    "match": {
...
- `triggers` — ключевые слова (`[ключ]`), активирующие обработку команды.
- `match` — только русские фразы (Vosk русская модель, английские
  шаблоны удалены осознанно). Это слова-действия (`_команда_`).
- `settings` — настройки (`{настройка}`): слово → множитель для
  параметра команды (см. раздел «Концепция команд» ниже).
- Параметр `{{step}}` в shell-команде подставляется из значения
  `"step"` команды с учётом настроек.
...
### 3. Концепция команд (commands.json)

Первый уровень обработки — `commands.json`. Вся распознанная речь —
это **поток строк** (каждый законченный фрагмент STT = строка).
Обозначения:

- `[ключ]` — ключевое слово: триггер (`алиса`, `пожалуйста`).
- `_команда_` — слово/словосочетание из `match`, указывающее на действие.
- `{настройка}` — слово/словосочетание, задающее параметр команды.

Правила поиска:

1. Ключ стоит **где угодно** в потоке — до или после команды.
2. Команда — слово/словосочетание **сразу перед или сразу после ключа**.
3. Команда определяется по **максимальному совпадению ≥ 90%** с
   `match`-шаблоном. **Не угадывать**: команды нужно произносить
   правильно; результат ниже порога — ложный вызов.
4. Слова **после команды** — настройки для неё.
5. Смотрим **три строки** потока: над ключом, с ключом, после ключа.
6. Если команды в текущей строке нет и она не была найдена ранее —
   **ждём следующую строку** и ищем там.
7. Нет команды вокруг ключа (ни над, ни с, ни под) — **ложный вызов, игнорируем**.

Примеры:

```
да блин                          [ключ]
какая же ты тупая [Алиса]       _сделай громче_ {немного}  ← команда после ключа

как же меня это достало          _выключи_                  ← команда до ключа
[Пожалуйста]

[Алиса] _сделай громче_          ← ключ и команда в одной строке
```

Текущий набор настроек:
- `громче`/`тише`: `{немного}` — шаг в 2 раза меньше; `{сильно}` —
  шаг в 2 раза больше (6% → 12%).

### 4. Триггерные слова и порядок обработки

Общая цепочка обработки:

1. Распознанная фраза → поиск команды по концепции выше (шаг `commands.json`)
2. Алиас из `aliases.json` → запуск (`[Alias]`)
3. LLM-классификатор (`intent`) — коверканья Vosk («ютюб»)
   разбирает он (`[Mini]`)
4. Скилл (`[Skill]`)
5. Console opencode (opencode-cli со скиллами) — финальный фолбэк
...
3. Фраза с ключом (триггером) → поиск команды по концепции выше
   (шаг 1, commands.json) → при совпадении ≥90% выполняется команда
   с настройками; без ключа — только вывод в консоль
...
pytest tests/test_commands.py::TestCommandMatcher::test_find_command -v
...
│   ├── commands.py         # CommandMatcher: ключ/команда/настройки (≥90%)
