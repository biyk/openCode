...
- Вопросы к LLM по триггерному слову («пожалуйста» / «алиса») — ответ озвучивается
...
Совпадение ищется как подстрока в распознанном тексте. Ключ `llm.history_limit` задаёт глубину истории диалога (читается в `main.py:98`).

### 3. Триггерные слова

Команды и LLM активируются только при наличии в реплике одного из триггеров из поля `triggers` (по умолчанию `["пожалуйста", "алиса"]`, через конфиг — в корне `commands.json`). Поиск регистронезависимый.

- Реплика **с триггером + команда** → выполняется команда
- Реплика **с триггером без команды** → запрос уходит в LLM, ответ озвучивается
- Реплика **без триггера** → текст выводится в консоль, ничего не выполняется
...
3. Распознанное слово с триггером → команда или LLM → озвучка (TTS); без триггера — только вывод в консоль
