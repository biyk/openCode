...
│   │   ├── cron_edit.py  # Правка расписания в cron/crontab.json: shift_job_schedule ставит заданиям по id «минута час * * *» от now+offset (атомарная запись); используется командой «я спать» для wake_video_and_volume; CLI python -m lib.scheduling.cron_edit
...
│   ├── check_types.py  # Неблокирующий запуск mypy по строгим зонам из mypy.ini (lib/voice_cmd, lib/core); --fail для будущей блокировки.
...
│   │   ├── test_wake_event.py  # Юнит-тесты команды «я проснулся»: выбор начавшегося «СНА», перенос только конца на сейчас, засчёт задачи «Пробуждение» и коды выхода CLI (без сети)
│   │   └── test_wake_replan.py  # Интеграция команды «я проснулся»: replan_day целиком на фейках — перенос событий из окна сна на окна после подъёма и автозаполнение остатка дня (без сети)
...
│   ├── speed/  # Скоростные замеры: обычные прогоны pytest их пропускают, запуск только с VOICE_SPEED=1
...
│   ├── test_check_blind_except.py  # Тесты проверки слепых except: детект блоков, доказательства, метка # blind-ok, режим --git.
...
├── mypy.ini  # Поэтапная типизация mypy: external-либы без стабов глушатся, строгость включается зонами (voice_cmd — typed defs, core — проверка без attr-defined).
