...
class TestTtsPlayback:
    ...
    def test_speak_sapi_script_missing(self):
        ...
        with patch("lib.synth.tts_playback._SAPI_SCRIPT", script):
    ...
    def test_speak_sapi_success(self):
        ...
        with patch("lib.synth.tts_playback._SAPI_SCRIPT", script):
            with patch("lib.synth.tts_playback.subprocess.run", side_effect=fake_run):
    ...
    def test_speak_sapi_empty_output(self):
        ...
        script.exists.return_value = True
        with patch("lib.synth.tts_playback._SAPI_SCRIPT", script):
            with patch("lib.synth.tts_playback.subprocess.run"):
                result = tts._speak_sapi("Привет")
    ...
    def test_play_file_unlink_error_tolerated(self):
        ...
        with open(path, "wb") as f:
            f.write(b"ID3")
        with patch("lib.synth.tts_playback.os.unlink", side_effect=OSError("файл занят")):
            tts._play_file(path)
        tts._play_mp3.assert_called_once_with(path, None)
    ...
    def test_play_wav_timeout(self, mock_popen, capsys):
        ...
        with patch("lib.synth.tts_playback.time.time", side_effect=[0, 0, 1000]):
            tts = TextToSpeech()
    ...
    @patch("lib.synth.tts_playback.subprocess.Popen", side_effect=Exception("boom"))
    ...
    def test_play_mp3_ffplay_missing(self, capsys):
        ...
        with patch("lib.synth.tts_playback.subprocess.Popen", side_effect=fake_popen):
