"""Тесты для lib/browser/youtube_browser.py (сценарии YouTube)."""
...
class TestYoutube:
    ...
    def test_youtube_play(self, mocker):
        ...
        tab = {"id": "1"}
        mocker.patch("lib.browser.youtube_browser.youtube_search", return_value=tab)
        mocker.patch(
            "lib.browser.youtube_browser.youtube_play_first_result", return_value=True
        )
        assert ytb.youtube_play("музыка") is True

    def test_youtube_play_no_tab(self, mocker):
        """youtube_play возвращает False без вкладки."""
        mocker.patch("lib.browser.youtube_browser.youtube_search", return_value=None)
    ...
    def test_youtube_open_first_success(self, mocker):
        ...
        mocker.patch("lib.browser.youtube_browser._YOUTUBE_FIRST_VIDEO_TIMEOUT", 30.0)
        mocker.patch("lib.browser.youtube_browser._YOUTUBE_EXTENSIONS_SETTLE", 0.0)
        mocker.patch("lib.browser.cdp_client.eval_js", return_value=(True, ""))
    ...
    def test_youtube_open_first_no_link(self, mocker):
        ...
        mocker.patch("lib.browser.cdp_client._activate")
        mocker.patch(
            "lib.browser.youtube_browser._youtube_first_video_link", return_value=""
        )
        mocker.patch("lib.browser.youtube_browser._YOUTUBE_FIRST_VIDEO_TIMEOUT", 0.1)
        mocker.patch("lib.browser.cdp_client.eval_js", return_value=(True, ""))
    ...
    def test_youtube_open_first_resumes_watch(self, mocker):
        ...
        mocker.patch("lib.browser_control.ensure_browser", return_value=True)
        mocker.patch("lib.browser.cdp_client._list_tabs", return_value=[existing])
        mock_open = mocker.patch("lib.browser_control.open_url")
        mocker.patch("lib.browser.cdp_client._activate")
    ...
    def test_youtube_open_first_prefers_watch_over_home(self, mocker):
        ...
        mocker.patch("lib.browser_control.ensure_browser", return_value=True)
        mocker.patch(
            "lib.browser.cdp_client._list_tabs",
        ...
        mock_resume = mocker.patch(
            "lib.browser.youtube_browser._youtube_resume_current", return_value=True
        )
        assert ytb.youtube_open_first() is True
