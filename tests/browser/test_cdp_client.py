"""Тесты для lib/browser/cdp_client.py (CDP-примитивы: HTTP, вкладки, JS)."""
...
class TestTabs:
    ...
    def test_list_tabs(self, mocker):
        ...
        mocker.patch(
            "lib.browser.cdp_client._http_request",
...
class TestIsRunning:
    ...
    def test_is_running_true(self, mocker):
        ...
        mocker.patch("lib.browser.cdp_client._http_request", return_value={})
        assert cdc.is_running() is True

    def test_is_running_false(self, mocker):
        """is_running возвращает False при исключении."""
        mocker.patch("lib.browser.cdp_client._http_request", side_effect=Exception("no"))
...
class TestClick:
    ...
    def test_click_found(self, mocker):
        ...
        mocker.patch("lib.browser.cdp_client.eval_js", return_value=(True, "__CLICKED__"))
        ok, value = cdc.click({}, "button")
        assert ok is True

    def test_click_not_found(self, mocker):
        """click вернёт False, если элемента нет."""
        mocker.patch("lib.browser.cdp_client.eval_js", return_value=(True, "__NOT_FOUND__"))
