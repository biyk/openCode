"""Тесты для lib/youtube_live.py (состояние плеера, открытие/закрытие)."""
...
class TestPlayerState:
    ...
    def test_paused(self, mocker):
        mocker.patch("lib.cdp_client.eval_js", return_value=(True, "paused"))
        assert yl._player_state({"id": "1"}) == "paused"
        assert yl.youtube_video_paused({"id": "1"}) is True

    def test_no_video(self, mocker):
        mocker.patch("lib.cdp_client.eval_js", return_value=(True, "no-video"))
    ...
    def test_eval_error_is_no_video(self, mocker):
        mocker.patch("lib.cdp_client.eval_js", return_value=(False, "err"))
        assert yl._player_state({"id": "1"}) == "no-video"
...
class TestOpenTestVideo:
    ...
    def test_open_success(self, mocker):
        mocker.patch("lib.cdp_client.open_new_tab", return_value={"id": "7"})
        mocker.patch.object(
    ...
    def test_open_no_tab(self, mocker):
        mocker.patch("lib.cdp_client.open_new_tab", return_value=None)
    ...
    def test_open_no_player_closes_tab(self, mocker):
        ...
        mocker.patch("lib.cdp_client.eval_js", return_value=(True, "no-video"))
        mock_close = mocker.patch("lib.cdp_client.close_tab")
