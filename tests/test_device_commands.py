...
class TestDeviceCommands:
    ...
    def test_commands_file_exists(self, device_commands_path):
        ...
        import os
        assert os.path.exists(device_commands_path), f"Commands file not found: {device_commands_path}"
    ...
    def test_find_command_with_platform(self, device_commands_path):
        ...
        result = matcher.find(first_pattern)
    ...
    def test_windows_commands_are_valid_powershell(self, device_commands_path):
        ...
        for cmd_id, cmd in commands.items():
            ...
            if windows_cmd and "powershell" in windows_cmd.lower():
                assert "Get-AudioDevice" not in windows_cmd, f"Command {cmd_id} uses unavailable Get-AudioDevice"
    ...
    def test_windows_addtype_is_valid(self, device_commands_path):
        ...
        for cmd_id, cmd in commands.items():
            ...
            if windows_cmd and "Add-Type" in windows_cmd:
                assert "VK_MEDIA_PLAY_PAUSE" not in windows_cmd, f"Command {cmd_id}: unused variable VK_MEDIA_PLAY_PAUSE"
                assert "VK_MEDIA_STOP" not in windows_cmd, f"Command {cmd_id}: unused variable VK_MEDIA_STOP"
                assert "public class " in windows_cmd, f"Command {cmd_id}: missing class definition"
                # Проверяем наличие .dll (может быть экранировано)
                normalized = windows_cmd.replace("\\", "")
                assert "user32.dll" in normalized or 'DllImport("user32' in normalized, f"Command {cmd_id}: missing .dll extension in DllImport"
