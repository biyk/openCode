...
from lib.runtime import media
from lib.runtime.media import is_media_available, is_media_playing
...
class TestMedia:
    ...
    def test_returns_true_when_playing(self, mocker):
        ...
        mocker.patch("lib.runtime.media.subprocess.run", return_value=proc)
    ...
    def test_returns_false_on_exception(self, mocker):
        ...
        mocker.patch("lib.runtime.media.subprocess.run", side_effect=OSError("no powershell"))
        assert is_media_playing() is False
    ...
    def test_available_true_when_session_exists(self, mocker):
        ...
        proc = subprocess.CompletedProcess(args=[], returncode=0, stdout=b"true\r\n")
        run_mock = mocker.patch("lib.runtime.media.subprocess.run", return_value=proc)
    ...
    def test_available_false_on_exception(self, mocker):
        """Исключение → False."""
        mocker.patch("lib.runtime.media.subprocess.run", side_effect=OSError("down"))
        assert is_media_available() is False
