...
- `lib/intent.py` — `IntentClassifier` (mini-LLM fallback for command detection); `detect(text, context=None)` — without context uses legacy media_probe prompt, with context uses triggers/statuses/requires/blocked prompt; `lib/media.py` — `is_media_playing()`; `lib/output.py` — `TranscriptionOutput` (print_text/print_info/print_debug/print_error — always pass output through this, providers announce via it).
